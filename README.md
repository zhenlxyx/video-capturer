...
|  |  | false | 隐藏窗口 |
| show_detail       | 在采集图像时显示采集记录，禁用可以使终端输出看起来简洁（如果设置了保存日志文件，则采集记录仍会保存到日志中）。 | true | 显示采集记录 |
|  |  | false | 隐藏采集记录 |
...
|  |  | false | 不保存日志 |
| input_type | 采集来源，可以指定视频文件或指定目录（不包括子文件夹）下的全部文件。 | "files" | 从 input_files 中采集 |
|  | | "folder" | 从 input_folder  中采集 |
| input_files | “输入”文件，即待采集的视频，可以在本地、可移动介质或网络上，可以是相对路径（如 output/file1.mp4）或绝对路径（如 D:/video-converter/output/file1.mp4）。 | ["到 file1.mp4 的路径"] | 采集 file1.mp4 |
|  | | ["到 file1.mp4 的路径", "到 file2.mp4 的路径"] | 采集 file1.mp4 和 file2.mp4 |
| input_folder | “输入”目录，即视频文件的所在位置，可以在本地、可移动介质或网络上，可以是相对路径（如 output/）或绝对路径（如 D:/）。 | "路径"，以 / 结尾 | 采集路径（不包括子文件夹）下的全部视频文件 |
| output_folder     | “输出”目录，即采集图像的保存位置，可以在本地、可移动介质或网络上，可以是相对路径（如 output/）或绝对路径（如 D:/）。 | "路径"，以 / 结尾 | 全部采集结果保存到路径下的“视频名称__设置名称”子文件夹中 |
| read_frames       | 视频文件的读法，即每几帧取一帧读。 | 1 | 逐帧读取视频 |
|  |  | n | 每 n 帧取一帧读 |
...
|  |  | ["frame", n, 0] | 按帧采集，采集 n 帧 |
|  |  | ["second", m, n] | 按秒采集，采集 m 秒，每 n 帧取一帧保存 |
...
在某些情况下，采集时对弹出的三个窗口进行拖拽会使采集暂停，直至鼠标松开。
...
答：可以，前提是这些位置须挂载到某一盘符（如 Z:/）下。挂载后，只须在 input_files 或 input_folder 中用绝对路径指定位置即可。如果视频文件的输入位置位于可移动介质或网络上，采集速度将受到影响。
